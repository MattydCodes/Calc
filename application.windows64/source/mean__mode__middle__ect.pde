...
void keyPressed(){
  ...
  if(key == ENTER){
    orderedList = appenedInt(orderedList,int(currentString));
    currentString = "";
    sortList();
    calcAll();
  }
...
boolean isenterable(String toenter){
  boolean found = false;
...
boolean inMode(int num, int[] array){
  boolean found = false;
...
int[] appenedInt(int[] array, int toappend){
